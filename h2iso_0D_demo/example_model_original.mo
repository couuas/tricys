...
// 流出逻辑（包括未燃烧物质和滞留物质的输运）
    ...
    parameter Real nonradio_loss[5](each unit = "1") = {0.0001, 0.0001, 0, 0, 0} "非放射性损失";
    parameter Real threshold = 200 "铺底量";
    parameter Real to_SDS_Fraction = 0.95;
    // 辅助变量：计算I的总和
    Real I_total "I的5个分量之和";
  equation
// 计算I的总和
...
// 根据储存量是否超过铺底量，分为两种情况
      if I_total > threshold then
        der(I[i]) = from_CPS[i] + from_WDS[i] + from_TES[i] - (1 + nonradio_loss[i])*(1 - threshold/I_total)*I[i]/T - decay_loss[i]*I[i];
        outflow[i] = (1 - threshold/I_total)*I[i]/T;
      else
        der(I[i]) = from_CPS[i] + from_WDS[i] + from_TES[i] - nonradio_loss[i]*I[i]/T - decay_loss[i]*I[i];
        outflow[i] = 0;
      end if;
// 输出流分配到SDS
