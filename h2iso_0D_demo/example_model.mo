package example_model
  model Column_0D
    Modelica.Blocks.Interfaces.RealInput feed[5] "进料 (g/h)" annotation(Placement(transformation(extent={{-120,-10},{-100,10}})));
    Modelica.Blocks.Interfaces.RealOutput top[5] "塔顶馏分 (g/h)" annotation(Placement(transformation(extent={{100,30},{120,50}})));
    Modelica.Blocks.Interfaces.RealOutput bottom[5] "塔釜馏分 (g/h)" annotation(Placement(transformation(extent={{100,-50},{120,-30}})));
    
    // 这些参数将由 override.txt 或 mos 脚本在仿真启动前动态注入
    parameter Real m_top_ref[5] = {0,0,0,0,0} "h2iso reference top product (g/h)" annotation(Evaluate=false);
    parameter Real m_bottom_ref[5] = {0,0,0,0,0} "h2iso reference bottom product (g/h)" annotation(Evaluate=false);
    parameter Real N_stages = 0 annotation(Evaluate=false);
    parameter Real R = 0 annotation(Evaluate=false);
    parameter Real T_top = 0 annotation(Evaluate=false);
    parameter Real T_bottom = 0 annotation(Evaluate=false);
    
    // 动态持液量 (Liquid Hold-up) 特征参数
    parameter Real tau = 1.0 "塔特征停留时间 (h) - 表征持液量延迟" annotation(Evaluate=false);
    Real I[5](start={0,0,0,0,0}) "塔内组分滞留量 (g)";
    Real outflow[5] "塔总输出流 (g/h)";
    
    Real SF_top[5] "塔顶分离系数";
  equation
    for i in 1:5 loop
      // 1. 动态质量守恒：引入物理状态导数表征塔内持液
      der(I[i]) = feed[i] - outflow[i];
      
      // 2. 物理迟滞：输出取决于当前持液量
      outflow[i] = I[i] / tau;
      
      // 3. 基于 h2iso 稳态结果的高保真组分分离 (分离对象从 feed 改为 outflow)
      SF_top[i] = if (m_top_ref[i] + m_bottom_ref[i] > 1e-12)
                  then m_top_ref[i] / (m_top_ref[i] + m_bottom_ref[i])
                  else 0;
      // 动态分离输出
      top[i] = SF_top[i] * outflow[i];
      bottom[i] = (1 - SF_top[i]) * outflow[i];
    end for;
  end Column_0D;

  model Equilibrator_0D
    Modelica.Blocks.Interfaces.RealInput feed[5] "进料 (g/h)" annotation(Placement(transformation(extent={{-120,-10},{-100,10}})));
    Modelica.Blocks.Interfaces.RealOutput outflow[5] "平衡器输出流 (g/h)" annotation(Placement(transformation(extent={{100,-10},{120,10}})));
    
    // 平衡器在5组分原子基准下严格守恒，含有一阶停留时间延迟
    parameter Real tau = 0.1 "平衡器特征停留时间 (h)" annotation(Evaluate=false);
    Real I[5](start={0,0,0,0,0}) "平衡器内组分滞留量 (g)";
  equation
    for i in 1:5 loop
      der(I[i]) = feed[i] - outflow[i];
      outflow[i] = I[i] / tau;
...
// 流出逻辑（包括未燃烧物质和滞留物质的输运）
  ...
  model O_ISS
    ...
    Modelica.Blocks.Interfaces.RealOutput to_SDS[5] "输出到SDS系统" annotation(
      Placement(transformation(origin = {110, -20}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {0, -114}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    // 输出端口：CD1 塔顶废气排放 (5维)
    Modelica.Blocks.Interfaces.RealOutput top_CD1[5] "CD1 塔顶富氢排放" annotation(
      Placement(transformation(origin = {110, 40}, extent = {{-10, -10}, {10, 10}})));

    // 实例化三塔与平衡器
    Column_0D CD1 annotation(Placement(transformation(extent={{-40,40},{-20,60}})));
    Column_0D CD2 annotation(Placement(transformation(extent={{0,0},{20,20}})));
    Column_0D CD3 annotation(Placement(transformation(extent={{40,-40},{60,-20}})));
    Equilibrator_0D equilibrator annotation(Placement(transformation(extent={{20,-20},{40,0}})));

  equation
    // Wang 2022 ISS-O 严格三塔级联拓扑：
    // 1. CD1 接收 WDS 进料与来自 CD2 塔顶的富氢/氘回流流 (CD2.top -> CD1)
    CD1.feed = from_WDS + CD2.top;
    
    // 2. CD2 接收 TES 增殖流、CPS 流、CD1 塔釜富同位素流及 CD3 塔顶回流流
    CD2.feed = from_TES + from_CPS + CD1.bottom + CD3.top;
    
    // 3. CD2 塔釜纯 HT 全量送入催化平衡器 (2HT -> H2 + T2)
    equilibrator.feed = CD2.bottom;
    
    // 4. 平衡器出口产物送入 CD3 浓缩高纯 T2
    CD3.feed = equilibrator.outflow;
