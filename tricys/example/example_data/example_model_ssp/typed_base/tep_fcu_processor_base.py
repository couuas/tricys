"""
Auto-generated strongly typed processor base class for TEP_FCU.
...
@dataclass
class TEP_FCUInputData:
    from_TEP_IP: List[float] = field(default_factory=list)


@dataclass
class TEP_FCUOutputData:
    to_I_ISS: Optional[List[float]] = None


class TEP_FCUProcessorBase(AbstractTrackProcessor):
    ...
    def process(self, request_vector: UnifiedStateVector) -> TrackResult:
        inputs = TEP_FCUInputData(
            from_TEP_IP=request_vector.boundary_inputs.get("from_TEP_IP", []),
        )
        outputs = TEP_FCUOutputData()
        ...
        out_dict = {}
        if outputs.to_I_ISS is not None:
            out_dict["to_I_ISS"] = outputs.to_I_ISS
