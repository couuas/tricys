"""
Auto-generated strongly typed processor base class for TEP_IP.
...
@dataclass
class TEP_IPInputData:
    from_TEP_FEP: List[float] = field(default_factory=list)


@dataclass
class TEP_IPOutputData:
    to_TEP_FCU: Optional[List[float]] = None


class TEP_IPProcessorBase(AbstractTrackProcessor):
    ...
    def process(self, request_vector: UnifiedStateVector) -> TrackResult:
        ...
        out_dict = {}
        if outputs.to_TEP_FCU is not None:
            out_dict["to_TEP_FCU"] = outputs.to_TEP_FCU
