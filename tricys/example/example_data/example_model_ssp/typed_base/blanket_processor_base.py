"""
Auto-generated strongly typed processor base class for Blanket.
...
@dataclass
class BlanketInputData:
    pulseInput: float = 0.0
    from_TES: List[float] = field(default_factory=list)


@dataclass
class BlanketOutputData:
    to_CL: Optional[List[float]] = None
    to_TES: Optional[List[float]] = None


class BlanketProcessorBase(AbstractTrackProcessor):
    ...
    def process(self, request_vector: UnifiedStateVector) -> TrackResult:
        ...
        out_dict = {}
        if outputs.to_CL is not None:
            out_dict["to_CL"] = outputs.to_CL
        if outputs.to_TES is not None:
            out_dict["to_TES"] = outputs.to_TES
